...
def wrapperkeyPressHandler(fig,ax, run): 
    def _keyPressHandler(event):
        if event.key in ['n', 'N', 'u', 'U','m','M','o','O','g','G']: 
            ...
            print("Number of edges in " + algo_str + " graph which are also in Concorde (Euclidean) TSP tour\t:", n_common_edges_tour)
            print("\nNumber of edges in " + "Concorde (Euclidean) TSP path\t\t\t\t:", len(tsp_path.edges))
            print("Number of edges in " + algo_str + " graph which are also in Concorde (Euclidean) TSP path\t:", n_common_edges_path)
            print(87*"-")
