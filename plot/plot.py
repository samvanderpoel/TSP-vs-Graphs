    ...
    if len(sys.argv) >= 2 and sys.argv[1] == '--file':
        ...
        with open(filename, 'r') as stream:
            try:
                ...
                # eps is for increasing the scaling factor so that
                # all points lie inside the unit box
                eps = 1.0
                M = max(xmax,ymax) + eps
        ...
        print(
            "Please run as one of:\n"
            "--> python src/main.py --interactive\n"
            "--> python src/main.py --file <file.yaml>\n"
...
def run_handler(points=[]):
    ...
    fig.subplots_adjust(right=1.00, left=0.00, bottom=0.00)
    ...
    ax.set_title(
        f'Points Inserted: {len(run.points)}',
...
def wrapperkeyPressHandler(fig,ax, run):
    ...
    def _keyPressHandler(event):
        if event.key in ['o', 'O']:
            ...
            compare_graphs = True
            print('Comparisons with Euclidean TSP activated.')
            ...
            if smpl_str == 'usqr':
                run.points = sorted(
                    pts_uni(numpts), key=lambda k: [k[0], k[1]]
                )

            elif smpl_str == 'uball':
                run.points = sorted(
                    pts_ball(numpts, r=1), key=lambda k: [k[0], k[1]]
                )

            elif smpl_str == 'bivar':
                sigma = float(
                    input("Enter standard deviation (typical value: 0.05): ")
                )
                run.points = sorted(
                    pts_normal(numpts, mu=0.5, sigma=sigma),
                ...
                sigma = float(
                    input(
                        "Enter standard deviation about each mode "
                        "(typical value: 0.05): "
                    )
                )
                run.points = sorted(
                ...
                run.points = sorted(
                    pts_annulus_random(numpts, r_inner=1.0, r_outer=2.0),
                    key=lambda k: [k[0], k[1]]
                )
            elif smpl_str == 'corners':
                run.points = sorted(
                    pts_corners(numpts, numpolyverts=4, s=0.7),
                    key=lambda k: [k[0], k[1]]
                )
            elif smpl_str == 'grid':
                run.points = sorted(
                    pts_grid(numpts), key=lambda k: [k[0], k[1]]
                )
            elif smpl_str == 'spokes':
                run.points = sorted(
                    pts_spokes(numpts), key=lambda k: [k[0], k[1]]
                )
            elif smpl_str == 'concen':
                numrings = int(input("Enter number of concentric rings: "))
                run.points = sorted(
                    pts_concentric_circular_points(numpts, numrings),
                    key=lambda k: [k[0], k[1]]
                )
            ...
            ax.set_title(
                f'Points generated: {len(run.points)}',
                fontdict={'fontsize':12}
            )
            applyAxCorrection(ax)
            fig.canvas.draw()
            ...
            ax.set_title(
                f"Graph Type: {geometric_graph.graph['type']}\n" + \
                f"Number of nodes: {len(run.points)}",
                fontdict={'fontsize':12}
            )
            ...
            if compare_graphs:
                ...
                n_common_edges_tour = num_common_edges(
                    tsp_tour,
                    geometric_graph
                )
                n_common_edges_path = num_common_edges(
                    tsp_path,
                    geometric_graph
                )
                print(
                    87*"-" + "\n"
                    f"Number edges in {algo_str} graph: {len(geometric_graph.edges)}\n\n"
                    f"Number of edges in Concorde (Euclidean) TSP tour: {len(tsp_tour.edges)}\n"
                    f"Number of edges in intersection of {algo_str} "
                    f"and Concorde (Euclidean) TSP tour: {n_common_edges_tour}\n\n"
                    f"Number of edges in Concorde (Euclidean) TSP path: {len(tsp_path.edges)}"
                    f"Number of edges in intersection of {algo_str} "
                    f"and Concorde (Euclidean) TSP path: {n_common_edges_path}\n" +
                    87*"-"
                )
