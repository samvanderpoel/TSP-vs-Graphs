# This file contains methods to generate various point
# distributions. Most of these are sourced from Jon Bentley's
# experimental TSP paper:
#
#       Bentley, Jon Jouis. "Fast algorithms for geometric
...
def pts_uni(numpts, xlim=[0,1], ylim=[0,1]):
    """
    Bentley #1. Uniform within the a rectangle of with specificed
    limits on x and y coordintes. default is unit-square
    """
...
def pts_annulus(
    ...
    """
    Place points on consecutive nested concentric circles, with
    points on each circle differing from previous circle by a
    scaling and rotation. If numrings is not a divisor of numpts,
    then not all circles will have the same number of points.
    """
...
def pts_clusnorm(numpts, numclus=10 , mu=0, sigma=0.05):
    ...
    for idx, pt in enumerate(cluspts):
        normal_pts_x = np.random.normal(
            loc=mu,
            scale=sigma,
            size=nums[idx]
        )
        normal_pts_y = np.random.normal(
            loc=mu,
            scale=sigma,
            size=nums[idx]
        )
        normal_pts = np.asarray(
            list(zip(normal_pts_x, normal_pts_y))
        )
        for n in normal_pts:
            pts.append(n+pt)
...
def pts_cubediam(numpts):
    """
    Bentley #5. x = y = U[0,1]. i.e. points are randomly
    chosen along the diagonal of a square
    """
...
def pts_corners(numpts, numpolyverts=4, s=0.7):
    """
    Bentley #7. Uniform distribution at the vertices of a
    regular polygon with numpolyverts vertices
    """
    ...
    vx = [
        2.0 * np.cos(k*2.0*np.pi/numpolyverts)
        for k in range(numpolyverts)
    ]
    vy = [
        2.0 * np.sin(k*2.0*np.pi/numpolyverts)
        for k in range(numpolyverts)
    ]
    pts = []
    for x, y, num in zip(vx, vy, nums):
        ...
        new_pts = [
            basept + pt
            for pt in np.asarray(list(zip(rx,ry)))
        ]
        pts.extend(new_pts)
...
def pts_grid(numpts):
    """
    Bentley #8. Choose numpts from a square grid that
    contains about 1.3*numpts points
    """
    # grid size is g x g (approx equal to) 1.3*N
    g = int(np.ceil(np.sqrt(1.3*numpts)))
    pts = []
    for x in range(g):
        for y in range(g):
            pts.append(np.asarray([x,y]))
...
def pts_spokes(numpts):
    ...
    pts1 = [
        np.asarray([r,0.5])
        for r in np.random.uniform(size=nh)
    ]
    pts2 = [
        np.asarray([0.5,r])
        for r in np.random.uniform(size=nh)
...
def pts_concentric_circular_points(numpts, numrings):
    ...
    is not a divisor of numpts, then not all concentric
    circles will have the same number of points.
    """
    div, rem = divmod(numpts, numrings)
    nums = numrings*[div]
    for i in range(rem):
    ...
    for ring in range(numrings):
        ...
        angles = [
            idx * 2*np.pi/nums[ring]
            for idx in range(nums[ring])
        ]
        xs = [
...
def perturb_pts(points, xptb=0.001, yptb=0.001):
    """
    Perturbing (uniformly) the x- and y-coordinates of points
    within intervals [x-xptb, x+xptb] and [y-yptb,y+yptb] for.
