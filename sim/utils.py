...
def graph_to_yaml(graph, dirname):
    ...
    Writes the coordinates of a nx graph to a yaml file.
        graph:
            nx graph
        dirname:
            dir in which to create anomalies dir and
            write yaml files
    """
    points = [
    ...
    filepath = os.path.join(dirname, f"{n}_{strnow}.yaml")
...
def compare(d, comp, g1, g2, anomalies, dirname):
    ...
    Compares two dictionaries g1, g2 of graphs indexed by iteration
        d:
            dict of graph intersection data by comparison type
        comp:
    ...
    new_data = []
    for i in range(n):
        num_edges = len(list(g1[i].edges()))
        common = num_common_edges(g1[i], g2[i]) / num_edges
        new_data += [common]
        if comp in anomalies:
            ...
            if criterion:
                graph_to_yaml(
                    g1[i],
                    os.path.join(dirname, "anomalies", comp)
                )
    d[comp] = new_data
