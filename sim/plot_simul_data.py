...
def read_simul_data(cloudtype, which_comps="all", subdir=""):
    ...
    for major_id in which_comps:
        all_comps = [
            "_".join([major_id, minor_id]) for minor_id in which_comps[major_id]
        ]
        fig, ax = plt.subplots(dpi=500)
        ax.set_title(
            "Comparison of edges in "
            + titles[major_id]
            + " and "
            + "Proximity Graphs\nSampling Type: "
            + sample[cloudtype],
        ...
        ax.legend(
            bbox_to_anchor=(1.05, 1), loc="upper left", title="Graphs", fancybox=True
        )
        ax.grid(color="gray", linestyle="--", linewidth=0.5)
        fig.savefig(
            plotsdir + "/" + cloudtype + "_" + major_id + "_vs_graphs.pdf",
            format="pdf",
            bbox_inches="tight",
            dpi=500,
        )
...
which_comps = {
    "tour": ["1nng", "2nng", "mst", "gab", "urq", "del", "path"],
    "path": ["1nng", "2nng", "mst", "gab", "urq", "del"],
    "bito": ["1nng", "2nng", "mst", "gab", "urq", "del"],
}
gdl_comps = {"tour": ["20pt", "1del", "2del"], "path": ["20pt", "1del", "2del"]}
